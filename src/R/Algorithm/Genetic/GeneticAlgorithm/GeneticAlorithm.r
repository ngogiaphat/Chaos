...
tournament_selection <- function(population, tournament_size = 3) {
  ...
  contestants <- sample(nrow(population), size = tournament_size, replace = FALSE)
...
crossover <- function(parent1, parent2) {
  ...
  crossover_point <- sample(1:nrow(parent1), size = 1)
  #Create a child by combining the 2 tails of 2 parents
  child <- rbind(parent1[1:crossover_point, ], parent2[(crossover_point + 1):nrow(parent2), ])
...
for (generation in 1:num_generations) {
    ...
    #The above code implements a simple genetic algorithm to find the x value that is close to the target value of the fitness function. Specifically, the algorithm will create an initial population, and then iterate for a given number of generations. 
    #In each generation, the algorithm will calculate the fitness of each individual in the population, then select individuals to cross and mutate to create a new generation. 
    #Then, the algorithm selects the best individuals to become the next generation's population. 
    #Finally, the algorithm returns the final best fitness value. 
    #In the above code, the target value of the fitness function is set to 5, and other parameters of the algorithm such as population size, number of generations and mutation rate are also set.
    #Selection of individuals in a population for hybridization
    parents <- replicate(pop_size/2, {
        #Perform selection and crossover to create new children
        child1 <- crossover(select(population, fitness, tournament_size), select(population, fitness, tournament_size), crossover_rate)
        child2 <- crossover(select(population, fitness, tournament_size), select(population, fitness, tournament_size), crossover_rate)
