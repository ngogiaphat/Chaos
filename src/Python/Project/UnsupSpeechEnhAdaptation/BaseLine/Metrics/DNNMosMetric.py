...
def compute_dnsmos(audio, fs=16000, input_length=9, _sessions={}):
    ...
    if not _sessions:
        _sessions.update({ 'sig': ort.InferenceSession(os.path.join(base, SIG_MODEL_PATH)), 'bak_ovr': ort.InferenceSession(os.path.join(base, BAK_OVR_MODEL_PATH))})
    if len(audio) < 2*fs:
    ...
    return dict(sig_mos=np.mean(predicted_mos_sig_seg), bak_mos=np.mean(predicted_mos_bak_seg), ovr_mos=np.mean(predicted_mos_ovr_seg))