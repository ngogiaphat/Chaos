...
class Dataset:
    ...
    def get_generator(self, batch_size=4, num_workers=4, pin_memory=True):
        generator_params = {'batch_size': batch_size, 'shuffle': self.augment, 'num_workers': num_workers, 'drop_last': True}
        return torch.utils.data.DataLoader(self, **generator_params, pin_memory=pin_memory)
    ...
    def safe_pad(self, tensor_wav, dtype=torch.float32):
        if self.zero_pad and tensor_wav.shape[0] < self.time_samples:
            ...
            padded_wav = torch.zeros(list(appropriate_shape[:-1]) + [self.time_samples], dtype=dtype)
            ...
            return tensor_wav
    @abstractmethod
    def get_arg_and_check_validness(self, key, choices = None, known_type = None, extra_lambda_checks = None):
        try:
            value = self.kwargs[key]
        except Exception as e:
            print(e)
            raise KeyError("Argument: <{}> does not exist in pytorch" "dataloader keyword arguments".format(key))
        if known_type is not None:
            if not isinstance(value, known_type):
                raise TypeError("Value: <{}> for key: <{}> is not an " "instance of " "the known selected type: <{}>" "".format(value, key, known_type))
        if choices is not None:
            if isinstance(value, list):
                if not all([v in choices for v in value]):
                    raise ValueError("Values: <{}> for key: <{}>" "contain elements in a" "regime of non appropriate " "choices instead of: <{}>" "".format(value, key, choices))
            else:
                if value not in choices:
                    raise ValueError("Value: <{}> for key: <{}> is " "not in the" "regime of the appropriate " "choices: <{}>" "".format(value, key, choices))
