...
class Dataset(torch.utils.data.Dataset, abstract_dataset.Dataset):
    ...
    def __init__(self, **kwargs):
        ...
        self.split = self.get_arg_and_check_validness('split', known_type = str, choices=['dev', 'test'])
        ...
        for idx, fname in enumerate(sorted(self.available_speech_filenames)):
            ...
            for j in range(3):
                if idx / self.n_samples <= self.n_speakers_cdf[j]:
                    this_n_sources = j + 1
                    break
            this_noise_path = os.path.join(self.noise_dataset_dirpath, self.available_noise_filenames[idx])
    ...
    def __getitem__(self, idx):
        ...
        new_speaker_mix_tensor, new_noise_tensor = self.mix_2_with_specified_snr(speaker_mix, noise_tensor, snr_ratio=sampled_input_snr)
