...
class Dataset(torch.utils.data.Dataset, abstract_dataset.Dataset):
    ...
    def __init__(self, **kwargs):
        ...
        self.use_vad = self.get_arg_and_check_validness('use_vad', known_type=bool)
        self.get_only_active_speakers = self.get_arg_and_check_validness('get_only_active_speakers', known_type=bool)
        self.split = self.get_arg_and_check_validness('split', known_type=str, choices=['dev', 'eval', 'train'])
    ...
    def wavread(self, path):
        ...
        if self.sample_rate < fs:
            waveform = torchaudio.functional.resample(waveform, fs, self.sample_rate, resampling_method="kaiser_window")
    ...
    def __getitem__(self, idx):
        ...
        mixture_tensor = self.get_padded_tensor(mixture_w - mixture_w.mean(), start_index=start_index)
...
def test_generator():
    ...
    data_loader = Dataset(sample_rate = sample_rate, fixed_n_sources = fixed_n_sources, timelength = timelength, augment = 'train' in split, use_vad = use_vad,
        zero_pad = True, split = split, get_only_active_speakers = get_only_active_speakers, normalize_audio = False, n_samples = -1)
