...
class Dataset(torch.Utils.data.Dataset, AbstractDataset.Dataset):
    ...
    def __init__(self, **kwargs):
        ...
        self.kwargs = kwargs
        self.zero_pad = self.get_arg_and_check_validness('zero_pad', known_type=bool)
        self.normalize_audio = self.get_arg_and_check_validness('normalize_audio', known_type = bool)
        self.min_or_max = self.get_arg_and_check_validness('min_or_max', known_type=str, choices = ['min', 'max'])
        self.split = self.get_arg_and_check_validness( 'split', known_type=str, choices = ['dev', 'test', 'train-100', 'train-360'])
        self.n_samples = self.get_arg_and_check_validness( 'n_samples', known_type = int, extra_lambda_checks = [lambda x: x >= -1])
        self.timelength = self.get_arg_and_check_validness( 'timelength', known_type=float)
        self.fixed_n_sources = self.get_arg_and_check_validness( 'fixed_n_sources', known_type = int, choices = [-1, 1, 2, 3])
        self.augment = self.get_arg_and_check_validness('augment', known_type = bool)
        self.n_speakers_priors = self.get_arg_and_check_validness('n_speakers_priors', known_type = list, extra_lambda_checks = [lambda y: (sum(y) - 1.) < 1e-5, lambda y: len(y) == 3])
        self.n_speakers_cdf = np.cumsum(self.n_speakers_priors).tolist()
        self.sample_rate = self.get_arg_and_check_validness('sample_rate', known_type = int, choices = [8000, 16000])
        ...
        self.available_filenames = [ os.path.basename(f) for f in glob2.glob(os.path.join(self.dataset_dirpath, 's1') + '/*.wav')]
    ...
    def get_path(self):
        path = os.path.join(LIBRI3MIX_ROOT_PATH, 'wav{}k'.format(int(self.sample_rate / 1000)), self.min_or_max, self.split)
    ...
    def simulate_a_source_in_a_room(self, waveform, room_params):
        ...
        room = pra.ShoeBox(room_dims, fs = int(self.sample_rate), materials = pra.Material(e_absorption), max_order = max_order)
        ...
        source_z_loc = np.random.uniform(self.source_h_region[0], self.source_h_region[-1])
        room.add_source([source_x_loc, source_y_loc, source_z_loc], signal = waveform, delay = 0.0)
        room.simulate()
